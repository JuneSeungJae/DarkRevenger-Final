...
### 1. 릴리즈(Releases) 빌드 파일로 실행하기 (추천)
프로젝트 저장소에 업로드된 빌드 파일을 다운로드하여 Unity 엔진 없이 곧바로 게임을 실행할 수 있습니다.

1. 본 GitHub 저장소 우측의 **Releases** 메뉴를 클릭하여 이동합니다.
2. 등록된 최신 버전("1.0v")의 **Assets** 영역에서 빌드 파일(`.zip`)을 다운로드합니다.
3. 다운로드가 완료되면 압축 파일의 **압축을 완전히 해제**합니다.
4. 해제된 폴더 내부에서 게임 실행 파일(`.exe`)을 더블클릭하면 바로 게임이 시작됩니다.
   * *주의: 폴더 내의 리소스 파일들이 함께 있어야 정상 구동되므로, 실행 파일만 별도로 다른 곳으로 옮기지 마세요.*

### 2. Unity 에디터에서 소스 구동하기
개발 및 에디터 상에서 직접 프로젝트를 빌드하고 구동하려면 아래의 환경 설정이 필요합니다.
...
* **구동 방법**:
  ...
  3. 에디터 화면 상단의 **Play(▶) 버튼**을 누르면 개발 환경에서 즉시 플레이가 가능합니다.
